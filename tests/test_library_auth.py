...
def test_auth(app, client):
    ...
    assert response.status_code == 200, f"response.data = {response.data.decode()} response.status_code = {response.status_code}"


def test_wrong_auth1(client):
    response = client.post("/api/login", json = {"username": "test-wron-user", "password": "test-passwd"})
...
def test_add_user_to_queue(app, db, client):
    ...
    response = client.post("/api/account/add-user-to-queue", json = {"username": "test-user"}, headers = {"auth-token": token})
    assert db.users.find_one({"username": "test-user"})
    db.users.delete_one({"username": "test-user"})
    assert response.status_code == 200
